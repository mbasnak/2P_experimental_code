function [smoothed] = singleTrialVelocityAnalysis9mm_TO(data, sampleRate, sampleRate_new)
% This function processes the raw position data obtained from FicTrac to
% give back the velocity in degrees.

% It takes the rawData (data), the original sampling rate (sampleRate), 
% and the downsampling rate (sampleRate_new) as inputs
...
%% Take the derivative
    ...
    diff.Intx = gradient(deg.Intx).* sampleRate_new; %we multiply by 25 because we have downsampled to 25 Hz
    diff.Inty = gradient(deg.Inty).* sampleRate_new; 
    diff.angularPosition = gradient(deg.angularPosition).* sampleRate_new;
    
    %%add the x and y in deg/s
    diff.IntxDeg = gradient(deg.IntxDeg).* sampleRate_new; %we multiply by 25 because we have downsampled to 25 Hz
    diff.IntyDeg = gradient(deg.IntyDeg).* sampleRate_new;
