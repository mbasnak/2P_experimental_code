function [smoothed] = singleTrialVelocityAnalysis9mm_TO(data, sampleRate, sampRate_new)
...
%% Downsample the position data to match FicTrac's output
    ...
    downsampled.Intx = resample_new(unwrapped.Intx, sampRate_new, sampleRate); %For a 1000 rate acquisition frame rate from the NiDaq, downsampling to 25 Hz equals taking 1 every 40 frames
    downsampled.Inty = resample_new(unwrapped.Inty, sampRate_new, sampleRate); %For a 1000 rate acquisition frame rate from the NiDaq, downsampling to 25 Hz equals taking 1 every 40 frames  
    downsampled.angularPosition = resample_new(unwrapped.angularPosition, sampRate_new, sampleRate);
...
%% Smooth the data
    ...
    smoothed.Intx = smoothdata(downsampled.Intx, 'rlowess', sampRate_new); 
    smoothed.Inty = smoothdata(downsampled.Inty, 'rlowess', sampRate_new); 
    smoothed.angularPosition = smoothdata(downsampled.angularPosition, 'rlowess', sampRate_new);
...
%% Take the derivative
    ...
    diff.Intx = gradient(deg.Intx).* sampRate_new;
    diff.Inty = gradient(deg.Inty).* sampRate_new; 
    diff.angularPosition = gradient(deg.angularPosition).* sampRate_new;
    
    %%add the x and y in deg/s
    diff.IntxDeg = gradient(deg.IntxDeg).* sampRate_new;
    diff.IntyDeg = gradient(deg.IntyDeg).* sampRate_new;
